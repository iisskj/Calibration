...
def load_pcd_to_ndarray(pcd_path):
    ...
    with open(pcd_path) as f:
        ...
        points = np.loadtxt(f)
        points = np.hstack((points[:, 0:3], points[:, 6].reshape([-1,1])))
        return points 
...
def voxelize(pc, voxel_size):
    ...
    return np.hstack([np.asarray(downsampled_cloud.points), np.asarray(downsampled_cloud.colors) * 255.0])[:, :4]


def calc_reprojection_error(root, extrinsic_matrix, intrinsic_matrix, distortion, pointcloud_image_pair_list, visualize=False):
    """
    ...
    for idx in range(len(corners_world_list)):
        ...
        mean_RMSE_list.append(np.mean(RMSE))
        distance_list.append(distance)
        print('idx={}, NPE={:.10f}'.format(idx, np.mean(RMSE)))

        # img = cv2.resize(img, (img.shape[1] // 3, img.shape[0] // 3))
        if visualize:
            corners_reproj = np.squeeze(re_projection).astype(np.int32)
            # draw re-projection
            ...
            cv2.namedWindow('Re-Projection', cv2.WINDOW_NORMAL)
    ...
    return RMSE_norm, np.mean(RMSE_norm), mean_RMSE_list
...
def visualize_colored_pointcloud(pc):
    try:
        ...
        return
        # plot rgba points
    mlab.figure('pc', bgcolor=(0.05, 0.05, 0.05))    ### error
    # 构建lut 将RGB颜色索引到点
