...
import cv2
# from segmentation_reproduction import region_growing_kernel

from seg import segmentationext


parser = argparse.ArgumentParser()
parser.add_argument('--config', default='./configs/sample.yaml', type=str, help="config file path")
...
# if any data set's NPE is too large(>10), then remove it and calibrate again
MAX_NPE = configs['calibration']['MAX_NPE']
...
def locate_chessboard(input):
    ...
    segmentation = segmentationext.region_growing_kernel(
        pc,
...
def ransac_segment(pc):
    ...
    cloud.points = o3d.utility.Vector3dVector(pc[:, :3].astype(np.float32))
    cloud.colors = o3d.utility.Vector3dVector(np.expand_dims((pc[:, 3] / 255.0), 1).repeat(3, axis=1))
...
def calibration(keep_list=None, preprocess_hook=None, error_list=None):
    ...
    if error_list is not None:
        ...
        error_idx = [pointcloud_image_pair_list[i][0] for i in error_list]
        print('error_index:{}'.format(error_idx))

        pointcloud_image_pair_list = [pointcloud_image_pair_list[i] for i in range(len(pointcloud_image_pair_list)) if i not in error_list]
    ...
    for idx, (pc_file, img_file, ROI_file) in pointcloud_image_pair_list:
        ...
        pc = utils.load_pcd_to_ndarray(pc_file)
    ...
    # /2
    # process corner detection without multi tasks
    # detection_result = []
    # for idx, (pc, img) in enumerate(zip(pc_list, img_list)):
    #     # detect 3D and 2D corners
    #     detection_result.append(
    #         corner_detection_task(pc, img)
    #     )
    ...
    err, err_mean, NPE = utils.calc_reprojection_error(root, extrinsic_matrix, intrinsic_matrix, distortion, pointcloud_image_pair_list, visualize=True)
    
    return err, np.array(NPE)
...
if __name__ == '__main__':
    ...
    while True:
        cycle_number +=1
        # if any data set's NPE is too large(>10), then remove it and calibrate again
        _, NPE = calibration(keep_list=None, error_list=error_index)
        if max(NPE) > MAX_NPE:
            error_index = np.where(NPE > MAX_NPE)[0]
            print('cycle_number:{}'.format(cycle_number))
            
        else:
            break
        
        if cycle_number > 5 or len(NPE) < 4:
            print('cycle_number:{}'.format(cycle_number))
            break
